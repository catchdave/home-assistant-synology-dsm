...
class SynoApi:
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        LOGGER.debug("Initializing Synology DSM with %s", self.config_url)
    ...
    def _setup_api_requests(self) -> None:
        ...
        if not self._with_storage:
            LOGGER.debug(
                "Disable storage api from being updated or '%s'", self._entry.unique_id
            )
    ...
    def _update(self) -> None:
        """Update function for updating API information."""
        LOGGER.debug("Start data update for '%s' (%s)", self._entry.unique_id, self.config_url)
